...
Consider the $396^(4k)$ term in the series. Suppose k=4. R 53-bit doubles can represent up to 22 significant digits:

```{r}
print(396^(4*4),digits=22)
```

Arbitrary-precision numbers are available with the excellent [Rmpfr](https://cran.r-project.org/web/packages/Rmpfr/vignettes/Rmpfr-pkg.pdf) package. 120-bit precision is more than enough to represent the above term exactly, i.e., an integer number with 42 digits:
